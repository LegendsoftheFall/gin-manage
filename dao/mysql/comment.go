...
func IsLeafComment(commentID int64) (isLeaf bool, err error) {
	...
	if err = db.Get(&leaf, sqlStr1, commentID); err != nil {
		return
	}
	if leaf == 0 {
		isLeaf = true
...
func HaveReplies(commentID int64) (have bool, err error) {
	...
	if err = db.Get(&reply, sqlStr, commentID); err != nil {
		return
	}
	if reply > 1 {
		have = true
...
func GetAncestorReply(commentID int64) (replies []int64, err error) {
	...
	err = db.Select(&replies, sqlStr, commentID)
	return
}
...
func IsReplyDeleted(commentID int64) (isDeleted bool, err error) {
	...
	if err = db.Get(&status, sqlStr, commentID); err != nil {
		return
	}
	if status == 0 {
		isDeleted = true
...
where item_id = ? and item_type = 2 and create_time < ? order by ? desc limit ?,?`
	if err = db.Select(&rootComment, sqlStr1, itemID, endTime, orderKey, (page-1)*size, size); err != nil {
		return
	}

	// 获取根评论总数
	sqlStr2 := `select count(1) from comments where item_id = ? and item_type = 2 and create_time < ?`
	err = db.Get(&total, sqlStr2, itemID, endTime)
	return
}
...
where comment_id in (
	...
	if err = db.Select(&replyComment, sqlStr1, rootCommentID); err != nil {
		return
	}
	...
	for _, reply := range replyComment {
		...
		if err = db.Get(&reply.ToReplyID, sqlStr2, reply.ReplyID); err != nil {
			continue
		}
		// 若是子评论
		...
		if err = db.Get(&reply.ToUserID, sqlStr3, reply.ReplyID); err != nil {
			continue
		}
	}
	return
