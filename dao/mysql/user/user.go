...
func InsertUser(user *model.User) (err error) {
	...
	_, err = mysql.DB.Exec(sqlStr, user.UserID, user.Email, user.Username, user.Password)
	return
}
...
func Login(user *model.User) (err error) {
	...
	if err = mysql.DB.Get(user, sqlStr, user.Email); err != nil {
...
func GetUserByID(id int64) (user *model.User, err error) {
	...
	if err = mysql.DB.Get(user, sqlStr, id); err != nil {
		if err == sql.ErrNoRows {
			err = ErrUserNotExist
		}
		return
...
func GetUserInfoByUserID(id int64) (userInfo *model.UserInfo, err error) {
	...
	if err = mysql.DB.Get(userInfo, sqlStr, id); err != nil {
		if err == sql.ErrNoRows {
			err = ErrUserNotExist
		}
		return
...
func GetFollowingUsers(p *model.ParamUserList) (userInfoList []*model.UserInfo, total int, err error) {
	...
	err = mysql.DB.Select(&userIDList, sqlStr1, p.CurrentUserID)
	if err != nil {
		return nil, 0, err
	}
	...
	err = mysql.DB.Select(&userInfoList, query, args...)
	return
}
...
func GetUserProfile(userID int64) (profile model.UserProfile, err error) {
	...
	if err = mysql.DB.Get(&profile, sqlStr, userID); err != nil {
		if err == sql.ErrNoRows {
			err = ErrUserNotExist
		}
		return
...
func UpdateUserProfile(userID int64, p *model.UserProfile) (err error) {
	...
	_, err = mysql.DB.Exec(sqlStr, p.UserName, p.Avatar, p.Location, p.Company, p.Position, p.Introduction, p.HomePage, p.Github, userID)
	return
}
...
func GetSearchUsers(p *model.ParamSearch) (userInfoList []*model.UserInfo, total int, err error) {
	...
	if err = mysql.DB.Select(&userInfoList, sqlStr1, p.Key, (p.Page-1)*p.Size, p.Size); err != nil {
		if err == sql.ErrNoRows {
