...
from article where author_id = ?  order by create_time desc limit ?,?`
	err = mysql.DB.Select(&articleList, sqlStr, id, (page-1)*size, size)
	if err != nil {
		return
	}
	return
...
func GetAllArticleListByID(id int64) (articleList []*model.ArticleInfo, err error) {
	sqlStr := `select article_id,title,subtitle,content,image,author_id,view_count,likes,comments,create_time
from article where author_id = ?  order by create_time desc`
	err = mysql.DB.Select(&articleList, sqlStr, id)
	if err != nil {
		return
	}
	return
...
func GetArticleNumByID(id int64) (number int, err error) {
	...
	err = mysql.DB.Get(&number, sqlStr, id)
	if err != nil {
...
from article where article_id = ?`
	err = mysql.DB.Get(&article, sqlStr, id)
	if err != nil {
		if err == sql.ErrNoRows {
			err = ErrInvalidID
		}
...
func GetUserIDByArticleID(id int64) (userID int64, err error) {
	...
	err = mysql.DB.Get(&userID, sqlStr, id)
	if err != nil {
		if err == sql.ErrNoRows {
			err = ErrUserNotExist
		}
...
from article where title like concat('%',?,'%') order by view_count desc limit ?,?`
	if err = mysql.DB.Select(&articleList, sqlStr, p.Key, (p.Page-1)*p.Size, p.Size); err != nil {
		if err == sql.ErrNoRows {
