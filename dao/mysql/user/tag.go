...
func GetTagDetailByID(id int) (detail *model.TagDetail, err error) {
	...
	if err = mysql.DB.Get(detail, sqlStr, id); err != nil {
		if err == sql.ErrNoRows {
			err = ErrInvalidID
		}
	}
...
func GetFollowingTags(p *model.ParamTagList) (tagList []*model.Tag, total int, err error) {
	...
	err = mysql.DB.Select(&tagNumList, sqlStr1, p.CurrentUserID)
	if err != nil {
		return nil, 0, err
	}
	...
	err = mysql.DB.Select(&tagList, query, args...)
	return
}
...
func GetTagIDsByArticleID(id int64) (tagIDs []int, err error) {
	...
	err = mysql.DB.Select(&tagIDs, sqlStr, id)
	return
}
...
func GetSearchTags(p *model.ParamSearch) (tagList []*model.Tag, total int, err error) {
	...
	if err = mysql.DB.Select(&tagList, sqlStr1, p.Key, (p.Page-1)*p.Size, p.Size); err != nil {
		if err == sql.ErrNoRows {
			zap.L().Warn("there is no tag in DB")
			err = nil
		}
