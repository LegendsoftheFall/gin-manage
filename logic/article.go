...
func EditArticle(p *model.Article) (err error) {
	p.Content = hooks.TrimHtml(p.Html)
	// 查询旧标签
	tagIDs, err := mysql.GetTagIDsByArticleID(p.ID)
	if err != nil {
		return
	}
	// 删除所有标签
	if err = mysql.DeleteTagByID(p.ID); err != nil {
		return
	}
	// 更新redis中的标签文章set
	...
	return mysql.UpdateArticle(p)
}

func DeleteArticle(id int64) (err error) {
	// 查询旧标签
	tagIDs, err := mysql.GetTagIDsByArticleID(id)
	if err != nil {
		return
	}
	// 删除所有标签
	if err = mysql.DeleteTagByID(id); err != nil {
		return
	}
	// 删除文章相关的点赞和收藏记录
...
func GetArticleByID(id, page, size int64) (articleList []*model.ArticleInfo, total int, err error) {
	articleList, err = mysql.GetArticleListByID(id, page, size)
	if len(articleList) == 0 {
		return nil, 0, err
	}
	total, err = mysql.GetArticleNumByID(id)
...
func GetSearchArticleList(p *model.ParamSearch) (ArticleList []*model.ApiArticleInfo, total int, err error) {
	...
	if p.Category == model.SearchTop {
		articleInfoList, total, err = mysql.GetSearchArticleListByView(p)
		if err != nil {
			zap.L().Error("mysql.GetSearchArticleListByView", zap.Error(err))
			return nil, 0, err
		}
	} else if p.Category == model.SearchLatest {
		articleInfoList, total, err = mysql.GetSearchArticleListByTime(p)
		if err != nil {
