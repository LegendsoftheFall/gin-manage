...
func GetAllTags(p *model.ParamTagList) (tagList []*model.Tag, total int, err error) {
	...
	for _, tag := range tagList {
		tag.IsFollow, err = user.IsFollowTag(p.CurrentUserID, tag.ID)
		if err != nil {
...
func GetHotTags(p *model.ParamTagList) (tagList []*model.Tag, total int, err error) {
	tagList, total, err = user.GetHotTags(p)
	if err != nil {
		zap.L().Error("mysql.GetAllTags failed",
			zap.Int64("userID", p.CurrentUserID),
			zap.Error(err))
...
func GetSearchTags(p *model.ParamSearch) (tagList []*model.Tag, total int, err error) {
	tagList, total, err = user.GetSearchTags(p)
	if err != nil {
		zap.L().Error("mysql.GetAllTags failed",
			zap.Int64("userID", p.CurrentUserID),
			zap.Error(err))
...
func GetFollowingTags(p *model.ParamTagList) (tagList []*model.Tag, total int, err error) {
	tagList, total, err = user.GetFollowingTags(p)
	if err != nil {
...
func GetTagInfo(p *model.ParamTagInfo) (tagDetail *model.TagDetail, err error) {
	// 获取tag信息
	tagDetail, err = user.GetTagDetailByID(p.TagID)
	if err != nil {
	...
	tagDetail.IsFollow, err = user.IsFollowTag(p.UserID, p.TagID)
