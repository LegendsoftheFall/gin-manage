...
func CreateComment(p *model.ParamComment) (commentID int64, err error) {
	...
	case model.ItemArticle:
		err = mysql.CreateRootComment(p)
	case model.ItemComment:
		err = mysql.CreateReplyComment(p)
	default:
		return
	}
	return
...
func DeleteComment(p *model.ParamDeleteComment) (err error) {
	...
	switch p.ItemType {
	case model.ItemArticle:
		err = mysql.DeleteRootComment(p.CommentID)
	case model.ItemComment:
		// 查询是否有回复
		isLeaf, err1 := mysql.IsLeafComment(p.CommentID)
		if err1 != nil {
			err = err1
			return
		}
		// 有回复，将评论状态设为0
		if !isLeaf {
			err = mysql.SetReplyComment(p.CommentID)
		} else {
			// 将本身加入待删除队列
			willDelete = append(willDelete, p.CommentID)
			// 若没有回复，查询所有祖先
			ancestors, err2 := mysql.GetAncestorReply(p.CommentID)
			if err2 != nil {
				err = err2
				return
			}
			...
			for _, c := range ancestors {
				//判断是否删除
				isDeleted, err3 := mysql.IsReplyDeleted(c)
				if err3 != nil {
					err = err3
					break
				}
				...
				if !isDeleted {
					...
					// 若删除，查询是否有多个回复
					haveReplies, err4 := mysql.HaveReplies(c)
					if err4 != nil {
						err = err4
						return
					}
			...
			for _, commentID := range willDelete {
				if err = mysql.DeleteReplyComment(commentID); err != nil {
					return
				}
			}
		}
...
func GetCommentList(p *model.ParamCommentList) (commentList []*model.ApiComment, total int, err error) {
	...
	rootComment, commentTotal, err := mysql.GetRootComment(p.ItemID, p.Page, p.Size, p.Order, time.Now().String())
	if err != nil {
	...
	for _, comment := range rootComment {
		...
		if !ok {
			userInfoMap[comment.UserID], err = mysql.GetUserInfoByUserID(comment.UserID)
			if err != nil {
		...
		// 获取根评论的所有回复
		replyComments, err1 := mysql.GetReplyComment(comment.CommentID)
		...
		for _, reply := range replyComments {
			...
			if !ok {
				userInfoMap[reply.UserID], err = mysql.GetUserInfoByUserID(reply.UserID)
				if err != nil {
					zap.L().Error("mysql.GetUserInfoByUserID",
			...
			if !ok {
				// 有回复对象
				if reply.ToUserID != 0 {
					userInfoMap[reply.ToUserID], err = mysql.GetUserInfoByUserID(reply.ToUserID)
					if err != nil {
						zap.L().Error("mysql.GetUserInfoByUserID",
